...
interface Props {
  /** Verified server-side from Clerk auth() — never undefined when this renders */
  clerkUserId: string
  /** Verified server-side from Clerk currentUser() */
  clerkEmail: string
}

export default function ProfileClient({ clerkUserId, clerkEmail }: Props) {
  const {
    user,
    setUser,
    loading,
    ...
    linkToClerk,
    dismissLegacyBanner,
  } = useProfileData(clerkUserId, clerkEmail)
